...
<body id="page-top">
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3 d-flex justify-content-between align-items-center">
                            <div>
                                <h6 class="m-0 font-weight-bold text-danger">Grid Edit Narahubung</h6>
                            </div>
                            <div>
                                @if (Auth::user()->role_id != 1)
                                @else
                        ...
                        <form action="{{ route('narahubung.update_grid_narahubung') }}" method="POST" enctype="multipart/form-data">
                            @method('PATCH')
                            @csrf
                            <div class="card-body">
                                            ...
                                            @forelse ($narahubung as $narahubungs)
                                                <tr>
                                                    <td><input type="text" placeholder="Narahubung ID"
                                                        class="form-control @error('inputs_narahubung.' . $narahubungs->id . '.id') is-invalid @enderror"
                                                        id="id"
                                                        name="inputs_narahubung[{{ $narahubungs->id }}][id]"
                                                        value="{{ old('inputs_narahubung.' . $narahubungs->id . '.id') ?? $narahubungs->id }}"
                                                        readonly></td>
                                                    <td><select class="form-select"
                                                            name="inputs_narahubung[{{ $narahubungs->id }}][donor_id]"
                                                            id="donor_id_{{ $narahubungs->id }}">
                                                            <option value="">--Pilih Donor--</option>
                                                            @foreach ($donorID as $id_donor)
                                                                <option value="{{ $id_donor->id }}"
                                                                    {{ $id_donor->id == old('inputs_narahubung.' . $narahubungs->id . '.donor_id', $narahubungs->donor_id) ? 'selected' : '' }}>
                                                                    {{ $id_donor->nama_organisasi }}</option>
                                                            @endforeach
                                                        </select>
                                                        @error('inputs_narahubung.' . $narahubungs->id . '.donor_id')
                                                            <div class="text-danger">{{ $message }}</div>
                                                        @enderror
                                                    </td>
                                                    <td>
                                                        <input type="text" placeholder="Nama Kontak"
                                                            class="form-control @error('inputs_narahubung.' . $narahubungs->id . '.nama_kontak') is-invalid @enderror"
                                                            id="nama_kontak"
                                                            name="inputs_narahubung[{{ $narahubungs->id }}][nama_kontak]"
                                                            value="{{ old('inputs_narahubung.' . $narahubungs->id . '.nama_kontak') ?? $narahubungs->nama_kontak }}">
                                                        @error('inputs_narahubung.' . $narahubungs->id . '.nama_kontak')
                                                            <div class="text-danger">{{ $message }}</div>
                                                        @enderror
                                                    </td>
                                                    <td><input type="text" placeholder=""
                                                            class="form-control @error('inputs_narahubung.' . $narahubungs->id . '.jabatan') is-invalid @enderror"
                                                            id="jabatan"
                                                            name="inputs_narahubung[{{ $narahubungs->id }}][jabatan]"
                                                            value="{{ old('inputs_narahubung.' . $narahubungs->id . '.jabatan') ?? $narahubungs->jabatan }}">
                                                        @error('inputs_narahubung.' . $narahubungs->id . '.jabatan')
                                                            <div class="text-danger">{{ $message }}</div>
                                                        @enderror
                                                    </td>
                                                    <td><input type="email" placeholder="Email"
                                                            class="form-control @error('inputs_narahubung.' . $narahubungs->id . '.email') is-invalid @enderror"
                                                            id="email"
                                                            name="inputs_narahubung[{{ $narahubungs->id }}][email]"
                                                            value="{{ old('inputs_narahubung.' . $narahubungs->id . '.email') ?? $narahubungs->email }}">
                                                        @error('inputs_narahubung.' . $narahubungs->id . '.email')
                                                            <div class="text-danger">{{ $message }}</div>
                                                        @enderror
                                                    </td>
                                                    <td><input type="tel" placeholder="Nomor Telepon"
                                                            class="form-control @error('inputs_narahubung.' . $narahubungs->id . '.no_telp') is-invalid @enderror"
                                                            id="no_telp"
                                                            name="inputs_narahubung[{{ $narahubungs->id }}][no_telp]"
                                                            value="{{ old('inputs_narahubung.' . $narahubungs->id . '.no_telp') ?? $narahubungs->no_telp }}">
                                                        @error('inputs_narahubung.' . $narahubungs->id . '.no_telp')
                                                            <div class="text-danger">{{ $message }}</div>
                                                        @enderror
                                                    </td>
                                                    <td><select class="form-select"
                                                            name="inputs_narahubung[{{ $narahubungs->id }}][status_id]"
                                                            id="status_id_{{ $narahubungs->id }}">
                                                            <option value="">--Pilih Status--</option>
                                                            @foreach ($status as $item_status)
                                                                <option value="{{ $item_status->id }}"
                                                                    {{ $item_status->id == old('inputs_narahubung.' . $narahubungs->id . '.status_id', $narahubungs->status_id) ? 'selected' : '' }}>
                                                                    {{ $item_status->name }}</option>
                                                            @endforeach
                                                        </select>
                                                        @error('inputs_narahubung.' . $narahubungs->id . '.status_id')
                                                            <div class="text-danger">{{ $message }}</div>
                                                        @enderror
                                                    </td>
                                                    <td>
                                                        <div class="d-inline-block">
                                                            <a class="btn btn-danger btn-circle delete-btn-narahubung"
                                                                onclick="deleteGridEditNarahubung()"
                                                                data-narahubung-id="{{ $narahubungs->id }}"
    ...
    <form id="deleteFormGridEditNarahubung" action="{{ url('grid_edit/'.$narahubungs->id.'/narahubung') }}" method="POST" style="display: none;">
        @method('DELETE')
        @csrf
    </form>
